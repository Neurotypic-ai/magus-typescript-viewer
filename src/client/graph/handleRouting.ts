...
/**
 * @deprecated Use `assignEdgeSides` + `MODULE_HANDLE_IDS` instead. Always
 * returns right/left regardless of direction; exists only to keep legacy
 * drilldown builders and tests compiling. New callers should never use it;
 * each edge now carries its own per-edge handle assignments.
 */
