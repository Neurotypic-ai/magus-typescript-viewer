...
- **`graph/`** — Dependency graph view-model / API payload shapes (`DependencyPackageGraph`, `ModuleStructure`, etc.)
  shared by the client assembler and UI. Vue Flow–specific types (`DependencyNode`, `GraphEdge`, `DependencyProps`,
  `SearchResult`) stay in `src/client/types/`.
